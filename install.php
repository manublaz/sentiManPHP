...
// Recoger mensajes flash de redirecciones previas
if (!empty($_SESSION['flash_exitos']) && is_array($_SESSION['flash_exitos'])) {
    $exitos = array_merge($exitos, $_SESSION['flash_exitos']);
    unset($_SESSION['flash_exitos']);
}
if (!empty($_SESSION['flash_exito'])) {
    $exitos[] = $_SESSION['flash_exito'];
...
if ($step === 2 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($test->connect_errno) {
        ...
        $test->close();
        // Mensaje de éxito que se mostrará en el paso 3 (vía sesión)
        $_SESSION['flash_exito'] = "✅ Conexión correcta al servidor MySQL (<code>$host:$port</code>, usuario: <code>$user</code>).";
        // Redirigir al paso 3 (GET) para que el usuario vea la página y confirme
...
if ($step === 3 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($con->connect_errno) {
        ...
        if (!$con->query("CREATE DATABASE IF NOT EXISTS `$f_db` CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")) {
            $errores[] = "❌ No se pudo crear la base de datos: " . $con->error;
        } else {
            $con->select_db($f_db);

            // Ejecutar install.sql (omitiendo CREATE DATABASE/USE que ya hicimos)
            $schemaFile = __DIR__ . '/install.sql';
            if (!file_exists($schemaFile)) {
                $errores[] = "❌ No se encuentra <code>install.sql</code>. Asegúrate de que está en la misma carpeta.";
            } else {
                $sqlSchema = file_get_contents($schemaFile);
                // Quitar comentarios y CREATE DATABASE/USE (los hacemos manualmente)
                $sqlSchema = preg_replace('/--[^\n]*\n/', "\n", $sqlSchema);
                $sqlSchema = preg_replace('/CREATE\s+DATABASE[^;]+;/i', '', $sqlSchema);
                $sqlSchema = preg_replace('/USE\s+`?[\w]+`?\s*;/i', '', $sqlSchema);

                // Separar por sentencias
                $sentencias = array_filter(array_map('trim', explode(';', $sqlSchema)));
                $okCount = 0;
                foreach ($sentencias as $s) {
                    if ($s === '' || stripos($s, 'COMMIT') === 0) continue;
                    if ($con->query($s)) {
                        $okCount++;
                    } else {
                ...
                if (empty($errores)) {
                    $exitos[] = "✅ Base de datos <strong>$f_db</strong> creada.";
                    $exitos[] = "✅ Tabla <strong>dictionary</strong> con todas las columnas de capas creada.";
                    $exitos[] = "✅ Tabla <strong>analisis_historico</strong> creada.";
                    $exitos[] = "✅ Vista <strong>v_estadisticas</strong> creada.";
                }
            }
        }
        $con->close();
...
if ($step === 4 && $_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!file_exists($sqlFile)) {
        ...
        if ($con->connect_errno) {
            ...
            if ($yaHay > 100) {
                $exitos[] = "ℹ️ El diccionario ya contiene <strong>$yaHay palabras</strong>. Se omite la importación para no duplicar datos.";
            } else {
                $sql = file_get_contents($sqlFile);
                // Quitar comentarios y USE
                $sql = preg_replace('/--[^\n]*\n/', "\n", $sql);
                $sql = preg_replace('/USE\s+`?[\w]+`?\s*;/i', '', $sql);

                // Soportar tanto INSERT como REPLACE
                preg_match_all('/(INSERT|REPLACE)\s+INTO[^;]+;/is', $sql, $m);
                $total = 0; $errCount = 0; $ultimoErr = '';
                foreach ($m[0] as $ins) {
                ...
                if ($errCount === 0) {
                    ...
                    $exitos[] = "⚠️ Importados $total registros con $errCount errores menores (probablemente duplicados).";
                } else {
                    $errores[] = "❌ No se pudo importar el diccionario. Último error: $ultimoErr";
                }
            }
            $con->close();
        }
...
?&gt;</div>
  ...
  <div class="card">
    ...
    <ul class="chk">
      ...
      <li><span class="ic">✅</span><span>Tabla <code>dictionary</code> con todas las columnas de capas (49 categorías) creada</span></li>
      <li><span class="ic">✅</span><span>Tabla <code>analisis_historico</code> creada</span></li>
      <li><span class="ic">✅</span><span>Diccionario con 7.000+ palabras y expresiones cargado</span></li>
